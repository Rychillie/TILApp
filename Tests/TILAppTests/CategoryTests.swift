...
final class CategoryTests: XCTestCase {
    ...
    func testCategoryCanBeSavedWithAPI() async throws {
        ...
        try await app.test(.POST, categoriesURI, beforeRequest: { request in
            try await request.content.encode(category)
        }, afterResponse: { response in
            let receivedCategory = try await response.content.decode(Category.self)
